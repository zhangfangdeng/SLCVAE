...
def get_bi_rnn_encode(embedding, f_cell, b_cell, sequence_length=None, scope=None, reuse=None):
    """
    Assumption, the last dimension is the embedding
    The second last dimension is the sentence length. The rank must be 3
    The padding should have zero
    ...
    # print embedding
    with tf.variable_scope(scope, 'RnnEncoding', reuse=reuse):
        _, encoded_input = tf.nn.bidirectional_dynamic_rnn(f_cell, b_cell, embedding, sequence_length=sequence_length, dtype=tf.float32)
